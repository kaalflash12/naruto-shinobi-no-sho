...
const BUILD="R41-AUTHORITATIVE-TERION-20260823";
...
async function authoritativeAction(req,env,ctx){
  ...
  const type=actionType(envelope);
  if(/grant|award|give.?xp|set.?level|admin/i.test(type))return json(req,env,400,{ok:false,error:"CLIENT_AUTHORITY_FORBIDDEN"});
  ...
  const member=members.find(x=>String(x.userId)===String(me.id));
  if(!member)return json(req,env,403,{ok:false,error:"ROOM_MEMBERSHIP_REQUIRED"});
  // Deliberadamente NÃO repassa difficulty/attribute/result do cliente ao resolvedor.
  const mechanicalResult=resolveTerionIntent({intent:{type},character:member.character||{}});
...
async function status(req,env,ctx){
  ...
  data.authority="server-terion-2d10";data.serverMechanicalResolution=true;data.clientDifficultyIgnored=true;data.emailLogin=true;data.buildAuthority=BUILD;
  data.routes={...(data.routes||{}),serverMechanicalResolution:true,emailLogin:true};
  return new Response(JSON.stringify(data),{status:res.status,statusText:res.statusText,headers:originHeaders(req,env)});
}
