...
// Este estágio corrige somente a classificação de handlers delegados que o resolvedor
// antigo confundia com emissores HTML por ambos conterem `data-action=`.
process.exitCode = 0;
...
for (const src of report.sourceFiles || []) {
  ...
  sourceText.set(src.path, fs.readFileSync(abs, 'utf8'));
}
...
function occurrences(text, needle) {
  const out=[];
  let at=0;
  while ((at=text.indexOf(needle,at))>=0) { out.push(at); at+=Math.max(1,needle.length); }
  return out;
}
...
function delegatedEvidence(value) {
  ...
  for (const [file,text] of sourceText) {
    for (const pos of occurrences(text,value)) {
      const a=Math.max(0,pos-1200), b=Math.min(text.length,pos+1600), ctx=text.slice(a,b);
      const selector=exactSelector.test(ctx);
      const eventBoundary=/addEventListener\s*\(|\.onclick\s*=|\.onchange\s*=|\.oninput\s*=|\.onsubmit\s*=|\.onkeydown\s*=|\.onpointerdown\s*=/.test(ctx);
      const selectorConsumer=/(?:closest|matches|querySelector|querySelectorAll)\??\.?(?:call)?\s*\(/.test(ctx);
      if(selector && eventBoundary && selectorConsumer){
        evidence.push({file,line:lineAt(text,pos),kind:'delegated-selector-handler'});
        continue;
      }
      if(explicitBranch.some(rx=>rx.test(ctx)) && eventBoundary){
        evidence.push({file,line:lineAt(text,pos),kind:'explicit-event-branch'});
      }
    }
  }
...
for(const unresolved of report.unresolvedUIActions||[]){
  ...
  if(found.length){
    ...
    trace.evidence=[...(trace.evidence||[]),...found];
  }
}
...
report.ok=pass;
report.uiHandlerResolution='delegated-selector-evidence-v2';
report.generatedAt=new Date().toISOString();
fs.writeFileSync(REPORT_PATH,JSON.stringify(report,null,2)+'\n');

// Mantém o Markdown canônico coerente com o relatório corrigido.
