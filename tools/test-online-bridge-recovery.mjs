...
function makeHarness({responseRoomId='room-1',responseOk=true}={}){
  ...
  const window={
    // Produção expõe snapshot clonado; a bridge não pode depender de mutar este retorno.
    __NARUTO_R41__:{state:()=>({online:clone(online)})},
...

{
  ...
  assert(recovered===true,'recovery should succeed from valid canonical room response even when data.online=false');
  ...
  assert(h.online.ready===false,'production-like cloned runtime snapshot must remain immutable from the bridge');
  const bridge=h.window.__SNS_ONLINE_BRIDGE_RECOVERY__.state().bridge;
  assert(bridge.ready===true,'bridge-owned readiness must become true after canonical room recovery');
  assert(bridge.roomId==='room-1','bridge-owned recovered room mismatch');
  assert(bridge.source==='recovery','bridge readiness must identify recovery as its authority');
  assert(!bridge.error,'successful recovery must clear bridge error');
  ...
  assert(h.window.__SNS_ONLINE_BRIDGE_RECOVERY__.version==='R41-ONLINE-BRIDGE-RECOVERY-20260825-V4','recovery version marker mismatch');
}

{
  ...
  const bridge=h.window.__SNS_ONLINE_BRIDGE_RECOVERY__.state().bridge;
  assert(bridge.ready===false,'mismatched room response must not mark bridge ready');
  assert(/ONLINE_ROOM_RECOVERY_INVALID/.test(bridge.error),'mismatched room response must expose safe diagnostic error');
}

{
  ...
  const bridge=h.window.__SNS_ONLINE_BRIDGE_RECOVERY__.state().bridge;
  assert(bridge.ready===false,'failed backend response must not mark bridge ready');
}
