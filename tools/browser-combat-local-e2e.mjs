...
const fixture={
  ...
  character:{
    ...
    village:'folha',originVillage:'folha',origin:'sem_cla',hijutsu:null,variant:null,dualRule:false,ruleMode:'adapted',class:'nin',element:'fogo',profession:'',
...
try{
  ...
  const persisted=await page.evaluate(key=>{try{return JSON.parse(localStorage.getItem(key)||'null')}catch{return null}},SAVE_KEY);
  ...
  if(!pp)throw new Error('lastCombatPresentation ausente no save persistido após v82-basic-melee');
