...
function parseJs(file,text){
  ...
  try{return {ast:acorn.parse(text,opts(first)),sourceType:first};}
    ...
    try{return {ast:acorn.parse(text,opts(second)),sourceType:second};}
      ...
      e.firstError=firstError.message;
      e.secondError=secondError.message;
      throw e;
    }
  }
}
  ...
  for(const [key,val] of Object.entries(node)){
    ...
    if(Array.isArray(val)){
      for(let i=0;i<val.length;i++){
        if(isNode(val[i]))walk(val[i],node,key,i,visit);
      }
    }else if(isNode(val)){
      walk(val,node,key,null,visit);
    }
  }
}
    ...
    for(const [key,val] of Object.entries(node)){
      ...
      if(Array.isArray(val)){
        for(const child of val){
          if(isNode(child))rec(child,false);
        }
      }else if(isNode(val)){
        rec(val,false);
      }
...
function propName(n,text){
  if(!n)return '';
  if(n.type==='Identifier'||n.type==='PrivateIdentifier')return n.type==='PrivateIdentifier'?'#'+n.name:n.name;
  const s=literal(n,text);
  if(s!==null)return s;
...
function contextName(node,parent,parentKey,index,text){
  ...
  if(parent?.type==='PropertyDefinition'&&parent.value===node)return propName(parent.key,text);
...
function refs(body){
  const cap=(re,g=1)=>{
    ...
    let m;
    re.lastIndex=0;
    while((m=re.exec(body)))out.push(m[g]);
    return uniq(out);
  };
  return {
...
function analyze(node,text){
  ...
  walkFunctionBody(node.body,n=>{
    if(n.type==='CallExpression'||n.type==='NewExpression'){
      const callee=exprName(n.callee,text);
      calls.push(callee);
      if(/addEventListener$/.test(callee)){
        const ev=literal(n.arguments?.[0],text);
        if(ev)eventTypes.push(ev);
      }
    }
  });
...
const candidates=(inv.files||[]).filter(f=>{
  const file=norm(f.path);
  return JS_EXT.has(path.extname(file).toLowerCase())&&!file.startsWith('docs/');
});
const functions=[];
const parsedFiles=[];
const failures=[];
...
for(const row of candidates){
  const file=norm(row.path);
  const text=read(file);
  let parsed;
  try{
    parsed=parseJs(file,text);
  }catch(e){
    failures.push({file,error:e.message,firstError:e.firstError,secondError:e.secondError});
    continue;
  }
  let count=0;
  walk(parsed.ast,null,null,null,(node,parent,parentKey,index)=>{
    ...
    const full=text.slice(node.start,node.end);
    const a=analyze(node,text);
    functions.push({
      file,
      ...
      name:contextName(node,parent,parentKey,index,text),
      kind:kindOf(node,parent),
      params:(node.params||[]).map(p=>patternName(p,text)).join(', '),
      calls:a.calls,
      refs:refs(body),
      ast:{
        parser:'acorn',
        parserVersion:acorn.version||null,
...
inv.astIndex={
  ...
  anonymousFunctions:functions.filter(f=>f.name.startsWith('<anonymous@')).length,
...
const md=[
  ...
  `Parser: ${code('Acorn '+(acorn.version||''))}. Delimitação por AST; regex não define início/fim de função.`,'',
  `- Arquivos JS candidatos: **${candidates.length}**`,
  `- Arquivos parseados: **${parsedFiles.length}**`,
  `- Falhas de parse: **${failures.length}**`,
  `- Funções/callbacks: **${functions.length}**`,
  `- Callbacks por contexto: **${inv.astIndex.callbackFunctions}**`,'',
  '| ID | Nome/contexto | Fonte | Tipo | SHA corpo |',
  '|---|---|---|---|---|'
];
for(const f of functions){
  md.push(`| ${f.id} | ${code(f.name)} | ${code(`${f.file}:${f.line}-${f.endLine}`)} | ${f.kind} | ${code(f.ast.bodySha256)} |`);
}
if(failures.length){
  md.push('','## Falhas de parse','');
  for(const f of failures)md.push(`- ${code(f.file)} — ${clean(f.error)}`);
}
...
console.log(JSON.stringify({
  ...
  parseFailures:failures.length,
  functions:functions.length,
  callbacks:inv.astIndex.callbackFunctions
},null,2));
