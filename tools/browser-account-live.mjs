...
try{
 ...
 await page.waitForSelector('[data-testid="sns-account-panel"]',{timeout:30000});assert((await page.evaluate(()=>window.SNS_ACCOUNT_UI?.build))==="ACCOUNT-UI-20260822-V1","account UI build ausente");checks.publicAccountUiLoaded=true;
 ...
 await page.reload({waitUntil:"domcontentloaded"});await page.waitForTimeout(2000);assert((await page.locator("#sns-account-button").innerText()).startsWith("Conta:"),"sessão não persistiu após reload");checks.sessionPersistsReload=true;
 await page.locator("#sns-account-button").click();await page.waitForSelector('[data-testid="sns-account-authenticated"]');await page.locator('[data-action="logout"]').click();await page.waitForSelector('[data-form="login"]');checks.logoutUi=true;
