...
try{
 assert(/^https:\/\//.test(apiOrigin),"API_ORIGIN Cloudflare ausente");
 const join=site.includes("?")?"&":"?";
 const resp=await page.goto(`${site}${join}api=${encodeURIComponent(apiOrigin)}&account=1&tab=register&v=${Date.now()}`,{waitUntil:"domcontentloaded",timeout:90000});assert(resp?.ok(),`site HTTP ${resp?.status()}`);
 ...
 assert((await page.evaluate(()=>window.__R41_GITHUB_API__?.backend))==="cloudflare-mongodb-durable-objects","frontend não está em Cloudflare/MongoDB");checks.cloudflareBackendSelected=true;
