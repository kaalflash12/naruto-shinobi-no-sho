...
const entryBase='cloudflare/r41-api/src/entry.js';
const entryAuthoritative='cloudflare/r41-api/src/entry-authoritative.js';
const wrangler='cloudflare/r41-api/wrangler.toml';
if(!exists(entryBase))fail(entryBase+' ausente');
if(!exists(wrangler))fail(wrangler+' ausente');
let selectedEntry='';
if(exists(wrangler)){
  const w=fs.readFileSync(path.join(ROOT,wrangler),'utf8');
  const m=w.match(/^main\s*=\s*["'](src\/(?:entry|entry-authoritative)\.js)["']/m);
  if(!m)fail('wrangler.toml deve apontar main somente para src/entry.js ou src/entry-authoritative.js');
  else selectedEntry='cloudflare/r41-api/'+m[1];
...
if(exists(entryBase)){
  const e=fs.readFileSync(path.join(ROOT,entryBase),'utf8');
...
if(selectedEntry===entryAuthoritative){
  ...
  if(!/authoritativeWorld/.test(e)||!/stripClientMechanical/.test(e))fail('entry-authoritative.js nao sanitiza persistencia de mundo/savepoint');
  if(!/worldMechanicalPayloadSanitized=true/.test(e)||!/worldSavepointCharacterFromMongo=true/.test(e))fail('entry-authoritative.js nao anuncia autoridade do mundo/savepoint');
  if(!/authoritativeWorldPersistence:true/.test(e))fail('entry-authoritative.js nao publica gate authoritativeWorldPersistence');
