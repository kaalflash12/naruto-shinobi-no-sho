...
try{
  ...
  assert(runtime.apiOrigin===apiOrigin,`Frontend não adotou Worker live. Esperado ${apiOrigin}; obtido ${runtime.apiOrigin}`);
  assert(runtime.backend==='cloudflare-mongodb-durable-objects',`Backend frontend inesperado: ${runtime.backend}`);

  status=await page.evaluate(async origin=>{
    const r=await fetch(`${origin}/api/status`,{headers:{accept:'application/json'}});
    let data=null;try{data=await r.json();}catch{}
    return{status:r.status,ok:r.ok,data,cors:r.headers.get('access-control-allow-origin')};
  },apiOrigin);
  assert(status.ok&&status.status===200,`CORS/status Worker falhou: ${JSON.stringify(status)}`);
  assert(status.data?.configured===true,`Worker não configurado: ${JSON.stringify(status.data)}`);
  assert(status.data?.cloudSave===true,'Worker live sem cloudSave MongoDB.');
  assert(status.data?.onlineRooms===true,'Worker live sem Durable Objects onlineRooms.');
  ...
  unauthorized=await page.evaluate(async origin=>{
    const r=await fetch(`${origin}/api/account/slots`,{method:'POST',headers:{'content-type':'application/json'},body:'{}'});
    let data=null;try{data=await r.json();}catch{}
  ...
  const report={
    ...
    scope:'PUBLIC_GITHUB_PAGES_TO_CLOUDFLARE_WORKER_MONGODB_ATLAS',
    gameURL:gameBase,apiOrigin,runtime,backendStatus:status,unauthorizedProbe:unauthorized,
    pageErrors,consoleErrors:consoleErrors.slice(0,50),failures
  };
