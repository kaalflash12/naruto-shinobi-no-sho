...
try{
  ...
  runtime=await page.evaluate(()=>({r41:!!window.__NARUTO_R41__,apiBuild:window.NARUTO_R41_API_BUILD||'',apiOrigin:window.NARUTO_R41_API_ORIGIN||''}));
  ...
  status=await page.evaluate(async origin=>{const r=await fetch(`${origin}/api/status`,{headers:{accept:'application/json'}});let data=null;try{data=await r.json();}catch{}return{status:r.status,ok:r.ok,data,cors:r.headers.get('access-control-allow-origin')}};},apiOrigin);
  ...
  unauthorized=await page.evaluate(async origin=>{const r=await fetch(`${origin}/api/account/slots`,{method:'POST',headers:{'content-type':'application/json'},body:'{}'});let data=null;try{data=await r.json();}catch{}return{status:r.status,data};},apiOrigin);
  ...
  const report={generatedAt:new Date().toISOString(),startedAt,status:failures.length?'FAIL_BROWSER_LIVE_API':'PASS_BROWSER_LIVE_API',ok:failures.length===0,scope:'PUBLIC_GITHUB_PAGES_TO_SUPABASE_EDGE',gameURL:gameBase,apiOrigin,runtime,backendStatus:status,unauthorizedProbe:unauthorized,pageErrors,consoleErrors:consoleErrors.slice(0,50),failures};
